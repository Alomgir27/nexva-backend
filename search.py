...
def generate_content_tags(title: str, content: str) -> list:
    """Generate semantic tags for content using LLM"""
    try:
        sample = content[:800]
...
async def extract_search_keywords(query: str) -> str:
    """Use LLM to extract the most relevant search keywords from user question"""
    try:
...
async def extract_query_tags(query: str) -> list:
    """Extract semantic tags from user query for tag-boosted search"""
    try:
...
async def search_chatbot_content(chatbot_id: int, query: str, max_results: int = 5):
    ...
    try:
        ...
        # Deduplicate and remove embeddings from results
        seen = set()
        unique_results = []
...
def cleanup_demo_data():
    """Remove demo products index if exists"""
    try:
